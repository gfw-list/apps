...
let result = {
  ...
  "YouTube": '<b>YouTube: </b>Checking Failed❗️',
  "Netflix": '<b>Netflix: </b>Checking Failed ❗️',
  "Dazn": "<b>Dazn: </b>Checking Failed ❗️",
  "Disney": "<b>Disneyᐩ: </b>Checking Failed ❗️",
  "Paramount" : "<b>Paramountᐩ: </b>Checking Failed ❗️",
  "Discovery" : "<b>Discoveryᐩ: </b>Checking Failed ❗️",
  "ChatGPT" : "<b>ChatGPT: </b>Checking Failed ❗️"
...
;(async () => {
  ...
  if (status == STATUS_COMING) {
      result["Disney"] = "<b>Disneyᐩ:</b> Coming Soon ➟ " + '⟦' + flags.get(region.toUpperCase()) + '⟧ ⚠️';
       ...
       result["Disney"] = "<b>Disneyᐩ:</b> <span style='color:green;'>Yes</span> "+arrow+ "⟦"+flags.get(region.toUpperCase())+"⟧ 🎉"
      console.log(result["Disney"]);
  } else if (status == STATUS_NOT_AVAILABLE) {
      result["Disney"] = "<b>Disneyᐩ:</b>   <span style='color:red;'>No</span> 🚫";
  } else if (status == STATUS_TIMEOUT) {
      result["Disney"] = "<b>Disneyᐩ:</b> TimeOut 🚦";
  }
  ...
  content = content + "</br>------------------------------</br>"+"<font color=#09cee0 >"+"<b>Node</b> ➟ " + $environment.params+ "</font>"
...
$configuration.sendMessage(message).then(resolve => {
    ...
    if (resolve.ret) {
      ...
      content = content + "</br>--------------------------------------</br>"+"<font color=#09cee0>"+"<b>Node</b> ➟ " + output+ "</font>"
      content =`<p style="text-align: center; font-family: -apple-system; font-size: large; font-weight: thin">` + content + `</p>`
...
.finally(() => {
  ...
  $configuration.sendMessage(message).then(resolve => {
    ...
    $done({"title":result["title"],"htmlMessage":`<p style="text-align: center; font-family: -apple-system; font-size: large; font-weight: thin">`+'----------------------</br></br>'+"🚥 Checking Error"+'</br></br>----------------------</br>'+ output + `</p>`})
}
  );
  ...
  return new Promise((resolve, reject) =>{
    ...
    $task.fetch(option).then(response => {
      ...
      if (response.statusCode === 404) {
        ...
        result["Netflix"] = "<b>Netflix: </b><span style='color:yellow;'>Original Only</span> ⚠️"
        ...
        //console.log("nfnf")
        result["Netflix"] = "<b>Netflix: </b><span style='color:red;'>No</span> 🚫"
        console.log("nf:"+result["Netflix"])
        ...
        result["Netflix"] = "<b>Netflix: </b><span style='color:green;'>Yes</span> "+arrow+ "⟦"+flags.get(region.toUpperCase())+"⟧ 🎉"
        //$notify("nf:"+result["Netflix"])
      ...
      result["Netflix"] = "<b>Netflix: </b>Timeout 🚦"
...
function testYTB() { 
    ...
    $task.fetch(option).then(response=> {
      ...
      if (response.statusCode !== 200) {
        //reject('Error')
        result["YouTube"] = "<b>YouTube: </b>Checking Failed ❗️"
      } else if (data.indexOf('Premium is not available in your country') !== -1) {
          //resolve('Not Available')
        result["YouTube"] = "<b>YouTube: </b><span style='color:red;'>No</span> 🚫"
      ...
      //resolve(region)
      result["YouTube"] = "<b>YouTube: </b><span style='color:green;'>Yes</span> "+arrow+ "⟦"+flags.get(region.toUpperCase())+"⟧ 🎉"
      console.log("ytb:"+region+ result["YouTube"])
      }
    }, reason => {
      result["YouTube"] = "<b>YouTube: </b>Timeout 🚦"
      //resolve("timeout")
    })
}
...
function testDazn() { 
  ...
  $task.fetch(option).then(response=> {
    ...
    if (response.statusCode !== 200) {
      //reject('Error')
      result["Dazn"] = "<b>Dazn: </b>Checking Failed ❗️"
      ...
      if (ret != null && ret.length === 2) {
        region = ret[1]
        result["Dazn"] = "<b>Dazn: </b><span style='color:green;'>Yes</span> "+arrow+ "⟦"+flags.get(region.toUpperCase())+"⟧ 🎉"
      } else {
        result["Dazn"] = "<b>Dazn: </b><span style='color:red;'>No</span>  🚫"
...
function testParam() { 
  ...
  $task.fetch(option).then(response=> {
    ...
    if (response.statusCode == 200) {
      //reject('Error')
      result["Paramount"] = "<b>Paramountᐩ: </b><span style='color:green;'>Yes</span> 🎉 "
    } else if (response.statusCode == 302) {
      //resolve('Not Available')
      result["Paramount"] = "<b>Paramountᐩ: </b><span style='color:red;'>No</span>  🚫"
    } 
  ...
  return new Promise((resolve, reject) =>{
    ...
    $task.fetch(option).then(response=> {
      ...
      $task.fetch(option1).then(response=> {
        ...
        if (locationd == "us") {
          result["Discovery"] = "<b>Discoveryᐩ: </b><span style='color:green;'>Yes</span> 🎉 "
          console.log("Supported Discoveryᐩ")
          resolve("Supported Discoveryᐩ")
          return
        } else {
          result["Discovery"] = "<b>Discoveryᐩ:</b> <span style='color:red;'>No</span> 🚫"
          console.log("Unsupported Discoveryᐩ")
          resolve("Unsupported Discoveryᐩ")
          return
        }
      }, reason => {
  ...
  return new Promise((resolve, reject) =>{
    ...
    $task.fetch(option).then(response=> {
      ...
      $task.fetch(option1).then(response=> {
        ...
        if (res != -1) {
          result["ChatGPT"] = "<b>ChatGPT: </b><span style='color:green;'>Yes</span> "+arrow+ "⟦"+flags.get(region.toUpperCase())+"⟧ 🎉"
          console.log("Supported ChatGPT")
          resolve("Supported ChatGPT")
          return
        } else {
          result["ChatGPT"] = "<b>ChatGPT: </b><span style='color:red;'>No</span> 🚫"
          console.log("Unsupported ChatGPT")
          resolve("Unsupported ChatGPT")
          return
        }
      }, reason => {
        console.log("Check-Error"+reason)
